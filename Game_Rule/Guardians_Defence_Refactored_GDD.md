...
### 3.1 村莊市集 (Village Market)
- **基礎卡牌區**：4 種基礎物資始終可用（正規軍、火把、長矛、乾糧）。
- **隨機供應區**：
  - **英雄**：隨機抽取 4 種不同的 Level 1 英雄。
  - **物品/法術**：隨機抽取 4 種不同的村莊物品或法術卡。
- **重新補貨**：每 3 個回合，隨機供應區會重新刷新一次。
...
2. **進入地城 (Enter Dungeon)**：
   ...
   - **戰鬥計算公式**：
     - `總光照 = 手牌所有光源卡總和`。
     - `光照不足懲罰 = Max(0, 區域懲罰值 - 總光照) * 1`。 (v3.15 校準)
     - `最終攻擊 = (英雄ATK + 武器ATK + 法術ATK) - 光照不足懲罰`。
     - `勇氣保底 (v3.16)`：若原始攻擊力 > 0，則最終攻擊力保底為 **1**，確保即使在極端黑暗中也能發揮基本戰意。
...
- 地城愈深愈黑暗。
- **計算**：每缺少 1 點光照，總物理攻擊力降低 1 點 (v3.15)。
- **勇氣保底 (v3.16)**：任何英雄組合只要具備戰鬥力，最終傷害保底為 1。
